...
class shortest_path(app_manager.RyuApp):
    ...
    def handle_tcp(self, dp, pkt):
        ...
        #self.logger.info(self.set)

        if ip_src == "10.0.0.1" and ip_dst == "10.0.0.2":


            if self.i == 0 and self.set == 1:

                #self.logger.info("info")
                #self.logger.info(self.i)
                #self.logger.info("set")
                #self.logger.info(self.set)
                self.logger.info("handle_tcp was called for s1")
                #self.logger.info(pkt)
                ...
                self.logger.info("value of self")
                self.logger.info(self.set)

            elif self.i == 1 and self.set == 1:
                #self.logger.info("info")
                #self.logger.info(self.i)
                #self.logger.info("set")
                #self.logger.info(self.set)
                self.logger.info("handle_tcp was called for s2")
                #self.logger.info(pkt)

                ofp = dp.ofproto
                ofp_parser = dp.ofproto_parser
                match = ofp_parser.OFPMatch(eth_type=ether.ETH_TYPE_IP, ipv4_src=ip_src, ipv4_dst=ip_dst, ip_proto=ip_proto,
                                            tcp_dst=dst_port, tcp_src=src_port)

                self.add_reactive_flow(self.dp4, match, 0, 100, 3)
                self.add_reactive_flow(self.dp2, match, 0, 100, 2)
                self.add_reactive_flow(self.dp5, match, 0, 100, 1)
                ...
                self.add_reactive_flow(self.dp4, match, 0, 100, 1)
                self.add_reactive_flow(self.dp2, match, 0, 100, 1)
                self.add_reactive_flow(self.dp5, match, 0, 100, 3)
                self.send_packet(self.dp5, 1, pkt)

                self.i = self.i + 1
                self.set = self.set -1

            elif self.i == 2 and self.set == 1:
                #self.logger.info("info")
                #self.logger.info(self.i)
                #self.logger.info("set")
                #self.logger.info(self.set)
                self.logger.info("handle_tcp was called for s3")
                #self.logger.info(pkt)
                ...
                self.add_reactive_flow(self.dp4, match, 0, 100, 4)
                self.add_reactive_flow(self.dp3, match, 0, 100, 2)
                self.add_reactive_flow(self.dp5, match, 0, 100, 1)
                ...
                self.add_reactive_flow(self.dp4, match, 0, 100, 1)
                self.add_reactive_flow(self.dp3, match, 0, 100, 1)
                self.add_reactive_flow(self.dp5, match, 0, 100, 4)
                self.send_packet(self.dp5, 1, pkt)
